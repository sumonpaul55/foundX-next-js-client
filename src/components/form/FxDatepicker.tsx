...
interface TInputProps extends IInputs { className?: string }
const FxDatepicker = ({ variant = "bordered", size = "md", required = true, name, label, className }: TInputProps) => {
    ...
    return (
        <Controller name={name} render={({ field: { value, ...fields } }) => {
            return <DatePicker label={label}  {...fields} variant={variant} size={size} isRequired={required} className={className && className} />
        }} />
    )
}
